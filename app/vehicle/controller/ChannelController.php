...
class ChannelController extends HomeBaseController
{
    ...
    public function channel( $channel_id = 0 ){
       /* if(empty($channel_id)){
        ...
        $channel_id = 1;
        $tpl_data = [];
        $tpl =  isset($tpl[$channel_id] )?$tpl_data[$channel_id]:'channel';
        $brand_name = 'GF';
        ...
        if(cmf_is_mobile()){
            return $this->fetch('channel/wap/'.$tpl);
        }
    ...
    border-right: 1px solid  #cdd1dc;" >%s</td>
                    ...
                    $storeName = !empty($store)?$store['name']:'广汽本田';

                    $book_car_style = '广汽本田 - '.getSeriesName($series_id);
                    if( $source_id == 1){
                        $source = '移动页面';
                    }elseif($source_id == 2){
                        $source = 'PC页面';
    ...
    private function _bookPageData($brand_name){
        $data =  file_get_contents(APP_PATH.'../data/conf/gb.json');
